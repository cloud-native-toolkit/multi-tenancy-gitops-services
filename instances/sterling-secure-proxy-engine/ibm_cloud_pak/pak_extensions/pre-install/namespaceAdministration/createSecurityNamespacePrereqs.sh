...
if supports_scc; then
    ...
    # Note: this script only works on OpenShift >= 3.11, otherwise you must run the following command manually
      oc adm policy add-scc-to-group ibm-ssp-engine-scc system:serviceaccounts:$namespace
    else
...
if supports_psp; then
    ...
    sed 's/{{ NAMESPACE }}/'$namespace'/g' ibm-ssp-engine-rb.yaml > $namespace-ibm-ssp-engine-rb.yaml
    ...
    kubectl create -f $namespace-ibm-ssp-engine-rb.yaml -n $namespace
