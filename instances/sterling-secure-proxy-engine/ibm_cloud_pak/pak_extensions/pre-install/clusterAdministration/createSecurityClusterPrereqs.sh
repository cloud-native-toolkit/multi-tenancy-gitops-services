...
if supports_scc; then
  ...
  kubectl create -f ibm-ssp-engine-scc.yaml --validate=false
  echo "Creating Cluster Role"
  kubectl create -f ibm-ssp-engine-cr-scc.yaml --validate=false
  isApplied=true
fi
...
if supports_psp; then
  ...
  if [ "$isApplied" == "false" ]; then
    ...
    kubectl create -f ibm-ssp-engine-psp.yaml
    echo "Creating Cluster Role"
    kubectl create -f ibm-ssp-engine-cr.yaml
  fi
fi
