...
if supports_scc; then
    ...
    if command -v oc >/dev/null 2>&1 ; then
        ...
        oc adm policy remove-scc-from-group ibm-ssp-engine-scc system:serviceaccounts:$namespace
    else
...
if supports_psp; then
    if [ "$isApplied" == "false" ]; then
		...
		sed 's/{{ NAMESPACE }}/'$namespace'/g' ../../pre-install/namespaceAdministration/ibm-ssp-engine-rb.yaml > $namespace-ibm-ssp-engine-rb.yaml

		# Delete the role binding for all service accounts in the current namespace
		kubectl delete -f $namespace-ibm-ssp-engine-rb.yaml -n $namespace

		# Clean up - delete the temporary yaml file.
		rm $namespace-ibm-ssp-engine-rb.yaml
	fi
fi
